...
class UserProfileUpdateViewTests(TestCase):
	...
	def test_profile_edit_includes_rich_bio_editor(self):
		...
		self.assertContains(response, "emoji-picker-element")
		self.assertContains(response, "data-rich-emoji-picker")
		self.assertContains(response, "emoji-click")
		self.assertContains(response, "activeTextColor")
		self.assertContains(response, "beforeinput")
		self.assertContains(response, "insertStyledText")
		self.assertNotContains(response, "EMOJI_CATEGORIES")
